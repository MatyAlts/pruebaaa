...
if (
  ...
  console.error(
    "Uso TEST explícito: node scripts/migrate-mobile-family.mjs --test-database <DB_NAME> --check|--apply",
  );
  process.exitCode = 1;
} else {
  ...
  try {
    ...
    if (mode === "--apply") await migrateFamilySchema(pool, name);
    console.log(
      JSON.stringify({
        database: name,
        status: (await familySchemaReady(pool))
          ? "ready"
          : "migration-required",
      }),
    );
